...
bool LcdStateSettings::pageSetTime() {
  ...
  lcd.move(0, 0);
  lcd << static_cast<char>(CustomCharacters::CLOCK) << PGM << STR_SET_CLOCK;
  lcd.move(2, 1);
...
bool LcdStateSettings::pageSetDate() {
  ...
  lcd.move(0, 0);
  lcd << static_cast<char>(CustomCharacters::CALENDAR) << PGM << STR_SET_DATE;
  lcd.move(2, 1);
...
bool LcdStateSettings::pageChargeTimer() {
  ...
  if (temp_buffer[0] == 0) {
    ...
    lcd << PGM << STR_ON << ' ' << stream::Time(temp_buffer[1], temp_buffer[2])
...
bool LcdStateSettings::pageKwhLimit() {
  ...
  lcd.move(0, 0);
  lcd << static_cast<char>(CustomCharacters::BATTERY1) << PGM
      << STR_SET_KWH_LIMIT;
